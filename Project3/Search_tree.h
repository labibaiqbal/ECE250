	...
	private:
		class Node {
			public:
				...
				//functions needed to transform BST to AVL tree
				void rotateLeft(Node*& current_node);
				void rotateRight(Node*& current_node);
				void balance (Node*& current_node);
				int balance_factor();
...
bool Search_tree<Type>::insert( Type const &obj ) {
	...
	if ( empty() ) {
		...
		//when theres's only the root_node the front_sentinel's next node will be root node and root_node's next_node will be back_sentinel
...
bool Search_tree<Type>::Node::insert( Type const &obj, Search_tree<Type>::Node *&to_this ) {
	//storing back_sentinel to backSentinel
	Search_tree<Type>::Node *next=to_this->next_node;

	if ( obj < node_value ) {
		if ( left_tree == nullptr ) {
			left_tree = new Search_tree<Type>::Node( obj );
			//when a new left tree is added it will become the previous node of to_this and next node of to_this's previous node.
		...
		if ( right_tree == nullptr ) {
			...
			update_height(); 
		//when a new right tree is added it will become the next node of to_this and previous_node of to_this->next_node.
	...
	if ( obj < node_value ) {
		if ( left_tree == nullptr ) {
			...
			if ( left_tree->erase( obj, left_tree ) ) {
				//balance after the erasing a node
				balance(to_this);
				update_height();
				return true;
			}
		...
		if ( is_leaf() ) {
			//making previous_node and next_node of to_this to point each other and deleting to_this
...
template <typename Type>
void Search_tree<Type>::Node::rotateLeft(Node*& current_node){
	Search_tree<Type>::Node *temp= current_node ->right_tree;
	current_node ->right_tree = temp->left_tree;
	temp->left_tree = current_node;
	current_node = temp;
}

template <typename Type>
void Search_tree<Type>::Node::rotateRight(Node*& current_node){
	Search_tree<Type>::Node *temp= current_node ->left_tree;
	current_node ->left_tree = temp->right_tree;
	temp->right_tree = current_node;
	current_node = temp;
...
template <typename Type>
void Search_tree<Type>::Node::balance (Node*& current_node){
	current_node ->update_height();
	if(current_node->balance_factor()< -1){
		if(current_node->left_tree->balance_factor()>0){
			rotateLeft(current_node->left_tree);
		}
		rotateRight(current_node);
	}else if(current_node ->balance_factor() >1){
		if(current_node->right_tree->balance_factor()<0){
			rotateRight(current_node->right_tree);
		}
		rotateLeft(current_node);
	}
	if(current_node->right_tree){
		current_node->right_tree->update_height();
	}
	if(current_node->left_tree){
		current_node->left_tree->update_height();
	}
	current_node->update_height();
}
...
int Search_tree<Type>::Node::balance_factor() {
	return (((right_tree)?right_tree -> tree_height+1:0)-((left_tree)?left_tree->tree_height+1:0));
}
